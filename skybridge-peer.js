 ...
 *
 * Usage:
 *   node skybridge-peer.js --device s24-ultra --port 9000 --peer tab-s9
 *   node skybridge-peer.js --device tab-s9 --port 9001 --peer s24-ultra
 *
 * What it does:
...
// ── Logging ───────────────────────────────────────────────────
const log  = (msg) => console.log(`[SkyBridge:${DEVICE_ID}] ${msg}`);
const warn = (msg) => console.warn(`[SkyBridge:${DEVICE_ID}] WARN: ${msg}`);
...
// ── Main ──────────────────────────────────────────────────────
async function main() {
  log(`Starting SkyBridge Peer Node`);
...
main().catch((e) => {
  console.error("[SkyBridge] Fatal:", e.message);
  process.exit(1);
});
